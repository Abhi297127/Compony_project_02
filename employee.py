...
def show_attendance_list(employee_id):
    """Show attendance in list format with detailed notes"""
    ...
    if 'attendance_list_edit_logs' not in st.session_state:
    ...
    if st.session_state.attendance_list_params != current_params:
        ...
        edit_logs_map = st.session_state.attendance_list_edit_logs if st.session_state.attendance_list_edit_logs else {}
    ...
    if attendance_records:
        ...
        for record in attendance_records:
                    ...
                    if reason and reason.strip():
                        ...
                        if edit_date:
                            if isinstance(edit_date, datetime):
                                edit_date_str = edit_date.strftime('%Y-%m-%d %H:%M')
                            else:
                                edit_date_str = str(edit_date)
                            edit_reasons.append(f"[{edit_date_str}] {edited_by}: {reason}")
                        else:
            ...
            display_data.append({
                ...
                'Created At': record.get('created_at', datetime.now()).strftime('%Y-%m-%d %H:%M') if record.get('created_at') else '-',
                'Last Updated': record.get('updated_at', datetime.now()).strftime('%Y-%m-%d %H:%M') if record.get('updated_at') else '-',
        ...
        for i, record in enumerate(attendance_records):
            ...
            if main_note or edit_history:
                with st.expander(f"{format_date_for_display(display_date)} - {record['status'].title()}"):
                    if main_note:
                        st.write(f"**Main Note:** {main_note}")
                    
                    if edit_history:
                        st.write("**Edit History:**")
                        for edit in edit_history:
                            old_status = edit.get('old_status', 'Unknown')
                            new_status = edit.get('new_status', 'Unknown')
                            reason = edit.get('reason', 'No reason provided')
                            edited_by = edit.get('edited_by', 'Unknown')
                            edited_at = edit.get('edited_at', '')
                            
                            if edited_at and isinstance(edited_at, datetime):
                                edited_at_str = edited_at.strftime('%Y-%m-%d %H:%M:%S')
                            else:
                                edited_at_str = str(edited_at) if edited_at else 'Unknown time'
                            
                            st.write(f"- **{edited_at_str}** by {edited_by}")
                            st.write(f"  Changed from '{old_status}' to '{new_status}'")
